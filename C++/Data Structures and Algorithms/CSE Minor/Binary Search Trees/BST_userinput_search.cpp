...
class TreeNode{
	...
	public:
		...
		TreeNode* findNode(int value){
			...
			for(int i=0;i<Max_children;i++){
				if(children[i]!=nullptr){
					TreeNode* found=children[i]->findNode(value);
					if(found!=nullptr){
						return found;
					}
				}
...
int main(){
	...
	cout<<endl;
	int key;
	cout<<"Searching a given value:"<<endl;
	cout<<"Enter the value of key"<<endl;
	cin>>key;
	TreeNode* result=root.findNode(key);
	if(result!=nullptr){
		cout<<"The given value has been found."<<endl;
	}
	else{
		cout<<"The given value cannot be found."<<endl;
	}
	return 0;
}
